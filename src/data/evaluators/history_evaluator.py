...
class HistoryEvaluator(DatasetEvaluator):
    def __init__(self):
        super().__init__()
        self.class_names = [
            "World",
    ...
    def max_class_toklen(self, llm_tokenizer: AutoTokenizer) -> int:
        ...
        tokenized_class_names = [
            llm_tokenizer(name, add_special_tokens=False).input_ids
            for name in self.class_names
        ]

        if self._max_class_toklen is None:
            self._max_class_toklen = max(
                len(class_name_toks) for class_name_toks in tokenized_class_names
            )

            # Print all class names with their token length
            for class_name, class_name_toks in zip(
                self.class_names, tokenized_class_names
            ):
