...
class PhotoDataset(BaseDataset):
    ...
    def process(self):
        ...
        graph.x = np.load(f"{raw_path}/Photo_roberta_base_512_cls.npy")
        ...
        class_names = ['Film Photography', 'Video', 'Digital Cameras', 'Accessories',
        ...
        QUESTION = f"Answer the following question: Which photography related subcategory does this description belong to? Please only output the most likely answer from the following subcategories and nothing else: {', '.join(class_names)} \n\n Answer:"
        
        # load the csv containing text data
        text_path = os.path.join(raw_path, "Photo.csv")
