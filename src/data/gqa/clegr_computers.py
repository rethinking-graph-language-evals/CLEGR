...
from src.data.gqa.generate_computers import generate_pyg_dataset, filter_question_forms
from src.data.gqa.args import get_args

from transformers import AutoTokenizer, AutoModel


class CLEGRComputersFactsDataset(BaseDataset):
    def __init__(
        self,
        root: str,
        transform=None,
        ...
        self.draw_dir = os.path.join(root, "clegr-computers-facts", "drawings")
        prefix = size+"-" if size != "small" else ""
        self.size = size
        self.seed = seed
        super().__init__(
            dataset_name=prefix + "clegr-computers-facts",
            root=root,
            transform=transform,
...
class CLEGRComputersReasoningDataset(BaseDataset):
    def __init__(
        self,
        root: str,
        transform=None,
        ...
        self.seed = seed
        super().__init__(
            dataset_name=prefix + "clegr-computers-reasoning",
            root=root,
            transform=transform,
            force_reload=force_reload,
        )
...
# diff size datasets
class TinyCLEGRFactsDataset(CLEGRComputersFactsDataset):
...
class LargeCLEGRFactsDataset(CLEGRComputersFactsDataset):
    def __init__(self, root: str, transform=None, force_reload: bool = False, seed: int = 42):
        super().__init__(
...
class TinyCLEGRReasoningDataset(CLEGRComputersReasoningDataset):
    def __init__(self, root: str, transform=None, force_reload: bool = False, seed: int = 42):
        super().__init__(
            root=root, transform=transform, force_reload=force_reload, size="tiny", seed=seed
        )
...
class LargeCLEGRReasoningDataset(CLEGRComputersReasoningDataset):
    def __init__(self, root: str, transform=None, force_reload: bool = False, seed: int = 42):
        super().__init__(
            root=root, transform=transform, force_reload=force_reload, size="large", seed=seed
        )
...
if __name__ == "__main__":
    data = CLEGRComputersFactsDataset("/scratch/djv4uc/datasets_transfer_final_grounded/", size="small")
    data.process()