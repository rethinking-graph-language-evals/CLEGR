...
from .types_ import NodeSpec, EdgeSpec, GraphSpec, LineSpec
    ...
    def __call__(self, graph: GraphSpec) -> Any:
        ...
        def ex(item):
            if isinstance(item, FunctionalOperator):
                return item(graph)
            # Handle NodeSpec/LineSpec arguments directly if they appear
            elif isinstance(item, (NodeSpec, LineSpec)):
                return item
            else:
                return item
...
#  The actual values (NodeSpec, LineSpec, str, etc.) are passed during generation.
# --------------------------------------------------------------------------
...
class Line(FunctionalOperator):
    # Represents a line argument. The actual LineSpec is passed in.
    pass
...
class Color(FunctionalOperator): pass
...
class Pluck(FunctionalOperator):
    def op(self, graph: GraphSpec, collection: List[Any], key: str):
        # Handles lists of NodeSpec, EdgeSpec, LineSpec, or dicts
        results = []
        for item in collection:
            try:
                if isinstance(item, (NodeSpec, EdgeSpec, LineSpec)):
...
class Pick(FunctionalOperator):
    def op(self, graph: GraphSpec, item: Any, key: str):
        # Handles NodeSpec, EdgeSpec, LineSpec, or dicts
        try:
            if isinstance(item, (NodeSpec, EdgeSpec, LineSpec)):
...
class Equal(FunctionalOperator):
    def op(self, graph: GraphSpec, a, b):
        # Special handling for comparing NodeSpec/LineSpec by ID
        if isinstance(a, (NodeSpec, LineSpec)) and isinstance(b, (NodeSpec, LineSpec)):
            return a.id == b.id
        return a == b
...
class CountIfEqual(FunctionalOperator):
    def op(self, graph: GraphSpec, l: List, t: Any):
        # Handle comparison with NodeSpec/LineSpec using ID
        count = 0
        for item in l:
            if isinstance(item, (NodeSpec, LineSpec)) and isinstance(t, (NodeSpec, LineSpec)):
                if item.id == t.id:
                    count += 1
            elif item == t:
                 count += 1
...
class Mode(FunctionalOperator):
    def op(self, graph: GraphSpec, l: List):
        ...
        # Handle unhashable types like NodeSpec/LineSpec by using their ID
        processed_list = []
        for item in l:
            if isinstance(item, (NodeSpec, LineSpec)):
        ...
        if len(most) == 1 or most[0][1] > most[1][1]:
             ...
             for item in l:
                 if isinstance(item, (NodeSpec, LineSpec)) and item.id == mode_val:
                     return item
                 elif not isinstance(item, (NodeSpec, LineSpec)) and item == mode_val:
                     return item
...
class Unique(FunctionalOperator):
    def op(self, graph: GraphSpec, l: List):
        # Handle NodeSpec/LineSpec based on ID for uniqueness
        seen_ids = set()
        unique_list = []
...
def GetLines(a: Any) -> FunctionalOperator: # a can be NodeSpec or List[NodeSpec]
    # Needs refinement: Edges(a) returns EdgeSpecs. Pluck needs 'line_name' or maybe LineSpec?
    # Let's return LineSpec objects for consistency
    ...
    # Better version returning LineSpec objects:
    # 1. Get Edges -> List[EdgeSpec]
    # 2. Pluck "line_id" -> List[str]
    # 3. Unique -> List[str]
    # 4. Map ID to LineSpec -> List[LineSpec] - This requires a new operator MapLineIdToLineSpec
...
class HasIntersection(FunctionalOperator):
    def op(self, graph: GraphSpec, list_a: List, list_b: List):
         # Use sets for efficient intersection checking, handle NodeSpec/LineSpec IDs
         set_a = set()
         for item in list_a:
             if isinstance(item, (NodeSpec, LineSpec)):
                 set_a.add(item.id)
...
class Intersection(FunctionalOperator):
    def op(self, graph: GraphSpec, list_a: List, list_b: List):
         ...
         # This is complex with mixed types and NodeSpec/LineSpec IDs.
         # Simplification: Assume inputs are primarily NodeSpec/LineSpec or simple hashables.
     ...
     def op(self, graph: GraphSpec, collection: List[Any], key: str, value: Any):
         ...
         for item in collection:
             try:
                 ...
                 # Handle NodeSpec/LineSpec comparison by ID if needed
                 should_include = False
                 if isinstance(item_value, (NodeSpec, LineSpec)) and isinstance(value, (NodeSpec, LineSpec)):
...
class Subtract(FunctionalOperator):
    def op(self, graph: GraphSpec, a, b):
        ...
        #     return None # Handle None gracefully, return None if either is None
...
class Max(FunctionalOperator):
    """
    Finds the maximum value from a list or multiple arguments.
    
    This operator can be called in two ways:
    1. With a single list argument: Max([item1, item2, ...])
    2. With multiple numerical arguments: Max(item1, item2, ...)
    """
    def op(self, graph: GraphSpec, *args):
        # Return negative infinity if no arguments are provided.
        if not args:
            return None

        items = []
        # Check if the call is with a single list or multiple arguments.
        ...
        processed_nums = []
        for item in items:
            # If an item is a list, use its length.
            if isinstance(item, list):
                processed_nums.append(len(item))
            # If an item is already a number, use it directly.
            elif isinstance(item, (int, float)):
                processed_nums.append(item)
        
        # Return a default value if the list of numbers is empty.
        if not processed_nums:
            return None

        # Return the maximum value from the processed numbers.
        return max(processed_nums)

#---------------------------------------------------------------------------------------------------
# class Min(FunctionalOperator):
#     def op(self, graph: GraphSpec, a, b):
#         if not (isinstance(a, (int, float)) and isinstance(b, (int, float))):
#             if isinstance(a, list): a = len(a)
#             if isinstance(b, list): b = len(b)
#             if not (isinstance(a, (int, float)) and isinstance(b, (int, float))):
#                 raise TypeError(f"Min requires numerical arguments, got {type(a)}, {type(b)}")
#         return min(a, b)
class Min(FunctionalOperator):
    """
    Finds the minimum value from a list or multiple arguments.
    
    This operator can be called in two ways:
    1. With a single list argument: Min([item1, item2, ...])
    2. With multiple numerical arguments: Min(item1, item2, ...)
    """
    def op(self, graph: GraphSpec, *args):
        # Return negative infinity if no arguments are provided.
        if not args:
        ...
        # Return the maximum value from the processed numbers.
        return min(processed_nums)

class EdgesBetween(FunctionalOperator):
    """
    Returns all EdgeSpec objects connecting two given stations (NodeSpec).
    """
    def op(self, graph: GraphSpec, s1: NodeSpec, s2: NodeSpec):
        if not isinstance(s1, NodeSpec) or not isinstance(s2, NodeSpec):
            raise TypeError("EdgesBetween expects NodeSpec arguments")
        # Find all edges where endpoints are s1 and s2 (order doesn't matter)
        # result = [
        #     edge for edge in graph.edges
        #     if (
        #         (edge.station1 == s1.id and edge.station2 == s2.id) or
        #         (edge.station1 == s2.id and edge.station2 == s1.id)
        #     )
        # ]
        for edge in graph.edges:
            if (edge.station1 == s1.id and edge.station2 == s2.id) or (edge.station1 == s2.id and edge.station2 == s1.id):
                return edge

class PluckLineProperty(FunctionalOperator):
    """
    For a list of EdgeSpec, pluck a property from the associated LineSpec for each edge.
    """
    def op(self, graph: GraphSpec, edges: list, property_name: str):
        results = []
        for edge in edges:
            line = graph.lines.get(edge.line_id)
            if line is not None:
                try:
                    results.append(line[property_name])
                except KeyError:
                    results.append(None)
            else:
                results.append(None)
...
class PathEdges(FunctionalOperator):
    """
    Given a list of nodes (NodeSpec), return the list of EdgeSpec objects connecting consecutive nodes.
    """
    def op(self, graph: GraphSpec, path: list):
        ...
        for i in range(len(path) - 1):
            ...
            # Find the edge connecting n1 and n2
            for edge in graph.edges:
                if (edge.station1 == n1.id and edge.station2 == n2.id) or (edge.station1 == n2.id and edge.station2 == n1.id):
                    edges.append(edge)
                    break
        return edges

class ParseInt(FunctionalOperator):
    """
    Converts a string to an integer.
    FIX: Added default for x=None to prevent missing argument errors.
    """
    def op(self, graph: GraphSpec, x=None):
        if x is None:
            return 0
        try:
            return int(x)
        except (ValueError, TypeError):
            return 0 # Default value for non-integer strings

class Map(FunctionalOperator):
    """
    Applies a function to each element of a list.
    NOTE: Changed 'func' type hint to Callable to support lambdas.
    """
    def op(self, graph: GraphSpec, l: list, func: Callable):
        return [func(x).op(graph, x) for x in l]

class GetLineEdges(FunctionalOperator):
    """
    Given a LineSpec, return all EdgeSpec objects belonging to that line.
    """
    def op(self, graph: GraphSpec, line: LineSpec):
        return [edge for edge in graph.edges if edge.line_id == line.id]
    
class GetLineNodes(FunctionalOperator):
    """
    Given a LineSpec, return all NodeSpec objects belonging to that line.
    """
    def op(self, graph: GraphSpec, line: LineSpec):
        ...
        for edge in line_edges:
            node_ids.add(edge.station1)
            node_ids.add(edge.station2)
        return [graph.nodes[node_id] for node_id in node_ids if node_id in graph.nodes]

class ArgMax(FunctionalOperator):
    """
    Given a list and a function, returns the element for which the function is maximized.
    """
    def op(self, graph: GraphSpec, items: list, func: Callable):
        if not items:
            raise ValueError("ArgMax called with empty list")
            
        max_item = None
        max_value = float('-inf')

        for item in items:
            value = func(graph, item)
            
            # FIX: If func returns an operator, execute it to get the actual value.
            if isinstance(value, FunctionalOperator):
                value = value(graph)
...
class SubgraphFromEdges(FunctionalOperator):
    """
    Given a list of EdgeSpec, return a NetworkX graph containing only those edges and their nodes.
    """
    def op(self, graph: GraphSpec, edges: list):
        subgraph = nx.Graph()
        for edge in edges:
            subgraph.add_edge(edge.station1, edge.station2)
        return subgraph

class CountConnectedComponents(FunctionalOperator):
    """
    Given a NetworkX graph, return the number of connected components.
    """
    def op(self, graph: GraphSpec, subgraph: nx.Graph):
        return nx.number_connected_components(subgraph)

class Flatten(FunctionalOperator):
    """
    Flattens a nested list structure (e.g., list of lists) into a single list.
    """
    def op(self, graph: GraphSpec, nested_list):
        result = []
