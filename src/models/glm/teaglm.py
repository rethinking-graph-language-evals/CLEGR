...
class TEA_GLM(BaseGLM):
    ...
    def __init__(
        ...
        llm_name = os.path.basename(llm.model_name)
        with open(f"{pretrain_path}_{llm_name}_model_params.json", "r") as f:
        ...
        state_dict = torch.load(
            f"{pretrain_path}_{llm_name}_model.pth", weights_only=False
        )
    ...
    def prepare_projector(self):
        projector_in_channels = self.gnn.n_classes
...
class TEA_GLM_LightningModule(pl.LightningModule):
    def __init__(
        self,
        model,
        lr=1e-3,
